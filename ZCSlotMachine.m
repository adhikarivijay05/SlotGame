...
static const NSUInteger kMinTurn = 1;
...
- (void)stopSliding
{
    ...
    for (CALayer *layer  in scrollingLayer) {
      //  [layer removeAllAnimations];
        CALayer *toBeAddedLayer = [CALayer layer];
        toBeAddedLayer.frame = layer.frame;
        toBeAddedLayer.contents = layer.contents;
        toBeAddedLayer.contentsScale = layer.contentsScale;
        toBeAddedLayer.contentsGravity = layer.contentsGravity;
        
        
        
        [layer removeAnimationForKey:@"slideAnimation"];
    }
...
- (void)startSliding {
    ...
    if (isSliding) {
        ...
        scrollingLayer = [[NSMutableArray alloc]init];
        for (int i = 0; i < [_slotScrollLayerArray count]; i++) {
            CALayer *slotScrollLayer = [_slotScrollLayerArray objectAtIndex:i];
            ...
            [scrollingLayer addObject: slotScrollLayer];
